    ...
    <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/80 backdrop-blur-sm dark:border-zinc-800 dark:bg-black/80">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        ...
        <nav
          ...
          className="hidden items-center gap-8 text-sm font-medium text-zinc-600 sm:flex dark:text-zinc-400"
        >
          {mainNav.map((item) => (
            <Link
              key={item.href}
      ...
      {open && (
        <nav
          ...
          className="border-t border-zinc-200 px-4 py-3 sm:hidden dark:border-zinc-800"
        >
            ...
            {mainNav.map((item) => (
              <li key={item.href}>
                <Link
                  ...
                  className="block rounded-md px-2 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-900"
                >
                  {item.label}
                </Link>
              </li>
